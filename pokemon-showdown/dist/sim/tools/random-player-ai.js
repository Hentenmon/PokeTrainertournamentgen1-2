    ...
    if (request.wait) {
      ...
      const choices = request.active.map((active, i) => {
        ...
        if (switches.length && (!moves.length || this.prng.next() > this.move)) {
          ...
          throw new Error(`${this.constructor.name} unable to make choice ${i}. request='${request}', chosen='${chosen}', (mega=${canMegaEvo}, ultra=${canUltraBurst}, zmove=${canZMove}, dynamax='${canDynamax}')`);
        }
      });
      this.choose(choices.join(`, `));
    } else {
