...
def parse_battles(file_path):
    trainer_stats = defaultdict(lambda: {
        'wins': 0, 'losses': 0, 'ties': 0, 'win_percentage': 0,
    })
    ...
    # Compute win percentage & best/worst opponents
    for trainer in trainer_stats:
        wins = trainer_stats[trainer]['wins']
        losses = trainer_stats[trainer]['losses']
        total_battles = wins + losses

        trainer_stats[trainer]['win_percentage'] = (
            (wins / total_battles) * 100 if total_battles > 0 else 0
        )
...
def save_to_csv(data, filename):
    with open(filename, mode='w', newline='') as file:
        ...
        writer.writerow(["Trainer", "Wins", "Losses", "Ties", "Win Percentage"])

        for trainer, record in data:
            writer.writerow([
                trainer, record['wins'], record['losses'], record['ties'],
                f"{record['win_percentage']:.2f}%"
            ])
...
for trainer, record in result:
      print(f"{trainer}: {record['wins']} Wins, {record['losses']} Losses, {record['ties']} Ties, "
      f"Win percentage: {record['win_percentage']}%")